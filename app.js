...
function generateCartSummary() {
  ...
  for (let index = 0; index < items.length; index++) {
    if (items[index].quantity != 0) {
      summary += `Item: ${items[index].name}, Quantity: ${items[index].quantity}\n`;
    }
  }
  summary += `Total: $${finalDollars}.${finalCents < 10 ? '0' : ''}${finalCents}`;
  return summary;
}
...
whatsappButton.onclick = () => {
  updatePrice();
  let summary = generateCartSummary();
  let whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(summary)}`;
